...
- **Type:** upgrade
- **Work done:**
  - DZ Hardware 7-pin LED relay installed (replaces OEM thermal relay)
  - Oxford Signal 1 resistors removed from all 4 turn signals
- **Parts used:**
  - LED relay 7-pin Suzuki — DZ Hardware (38-30797)
- **Cost:** 21.90
- **Shop/Self:** self
...
- **Type:** routine
- **Work done:**
  - Oil + filter changed (17mm drain bolt, crush washer reused/flipped, sight glass check)
  - Brake fluid flushed front (both calipers, left first) and rear (DOT4)
  - 2 of 4 spark plugs replaced (rear top, front right)
  - Air filter replaced
  - 4x Oxford Signal 1 LED turn signals installed with resistors
  - Rear ground junction cleaned under seat — fixed brake light flickering
  - Forks checked — no oil weep
  - Steering head bearings checked — no play
- **Parts used:**
  - Oil filter (spin-on, front right of engine)
  - 10W-40 oil — ~2.4L
  - DOT4 brake fluid
  - Air filter
  - 4x Oxford Signal 1 (EL301) — 3W, 18 LEDs, E-marked, M10, with resistors
  - 2x NGK CR8E
- **Shop/Self:** self
...
- **Type:** inspection
- **Work done:**
  - Battery tested: 12.83V rest, 14.5V running — healthy
  - Chain cleaned with WD-40, re-lubed
  - Rear brake confirmed fixed (from prior repair)
  - OEM 7-pin flasher relay tested — all pins probed with multimeter
  - Root cause identified: OEM thermal relay can't cycle with low-current LEDs
  - Tyre pressure set: 2.3 bar front / 2.5 bar rear (cold)
- **Parts used:** none
- **Shop/Self:** self
- **Notes:** Relay pin 6 drops to 7–8V fluctuating when blinkers activated. Pins 5, 7 are ignition-switched (fuel pump). Decided on Oxford Signal 1 + LED relay replacement approach.
